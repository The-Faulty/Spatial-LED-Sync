...
class EngineRuntime:
    def __init__(self, config: EngineConfig, logger: logging.Logger):
        ...
        self.wave_engine: WaveEngine | None = None
    ...
    def start(self) -> None:
        ...
        if spatial_topology is not None:
            self.wave_engine = VectorizedSpatialRenderer(self.config, spatial_topology)  # type: ignore[assignment]
        else:
            self.wave_engine = WaveEngine(self.config, self.topology)
    ...
    def step_once(self, timeout: float = 0.0) -> RuntimeSnapshot:
        ...
        try:
            ...
            for raw in raw_frames:
                ...
                if hasattr(self.wave_engine, "set_tv_frame"):
                    self.wave_engine.set_tv_frame(frame)  # type: ignore[attr-defined]
                analysis = self.motion.analyze(frame)
                self.analysis_frames += 1
                detected = self.event_detector.detect(analysis)
                log_events = [e for e in detected if e.kind != "front_ambient"]
                if log_events:
                    self.logger.info("events: %s", ", ".join(f"{e.kind}:{e.edge}:{e.intensity:.2f}" for e in log_events))
                ...
                if (
                    ...
                    and not isinstance(self.wave_engine, VectorizedSpatialRenderer)
                ):
        ...
        if self.config.overload_policy == "adaptive_quality":
            ...
            if self._analysis_tick != 0:
                ...
                if hasattr(self.wave_engine, "set_tv_frame"):
                    ...
                    self.wave_engine.set_tv_frame(prepared)  # type: ignore[attr-defined]
                return []
            return [frames[-1]]
